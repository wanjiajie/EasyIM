...
bool UserManager::loadUsersFromDb() {
    ...
    if(!pConn->initialize(m_strDbServer,m_strDbUserName,m_strDbPassWord,m_strDbName)) {
        LOGF("UserManager::loadUserFromDb failed,please check params: dbserver: %s, dbusername: %s, dbpassword: %s, dbname: %s",
    ...
    if(NULL == pResult) {
        LOGI("UserManager:: Query error,dbname: %s",m_strDbName.c_str());
        return false;
    }
    ...
    while(true) {
        ...
        //计算库中最大userid
        if(u.userid < GROUPID_BOUNDARY && u.userid > m_baseUserId) {
            m_baseUserId = u.userid;
        } 

        //计算库中最大群组id
        if(u.userid > GROUPID_BOUNDARY && u.userid > m_baseGroupId) {
    ...
    LOGI("current base userid: %d,current base groupid: %d",m_baseUserId.load(),m_baseGroupId.load());
...
bool UserManager::makeFriendRelationShipInDB(int32_t smallUserid,int32_t greaterUserid) {
    ...
    if(smallUserid > greaterUserid) {
        swap(greaterUserid,smallUserid);
    }
...
bool UserManager::releseFriendRelationshipInDBAndMemory(int32_t smallUserid,int32_t greaterUserid) {
    if(smallUserid == greaterUserid) {
        return false;
    }
...
bool UserManager::updateUserRelationshipInMemory(int32_t userid,int32_t target,FRIEND_OPERATION operation) {
    if(operation == FRIEND_OPERATION_ADD) {
        ...
        bool found2 = false;
        {
            ...
            for (auto& iter : m_allCachedUsers) {
                if(iter.userid == userid) {
                    for(auto iter_inner = iter.friends.begin(); iter_inner != iter.friends.end(); iter_inner++ ) {
                        if(iter_inner->friendid == target) {
                            iter.friends.erase(iter_inner);
                            found1 = true;
                            break;
                        }
                    }
...
bool UserManager::deleteFriendToUser(int32_t userid,int32_t friendid) {
    ...
    for(auto& iter : m_allCachedUsers) {
        if(iter.userid == userid) {
            for(auto iter_inner = iter.friends.begin(); iter_inner != iter.friends.end(); iter_inner++ ) {
                if(iter_inner->friendid == friendid) {
                    iter.friends.erase(iter_inner);
                    found1 = true;
                    break;
                }
...
bool UserManager::isFriend(int32_t userid,int32_t friendid) {
    ...
    for(auto& iter : m_allCachedUsers) {
        if(iter.userid == userid) {
            for(auto& iter_inner : iter.friends) {
                if(iter_inner.friendid == friendid) {
                    return true;
                }
            }
...
bool UserManager::updateUserInfoInDb(int32_t userid,const User& newuserinfo) {
    ...
    LOGE("Failed to update userinfo to db, find exist user in memory error, m_allCachedUsers.size(): %d, userid: %d, sql: %s", m_allCachedUsers.size(), userid, osSql.str().c_str());
...
bool UserManager::deleteTeam(int32_t userid,const std::string& deletedteamname) {
...

    {
        ...
        for(auto& iter : m_allCachedUsers) {
            if(iter.userid == userid) {
                for(auto& iter_inner : iter.friends) {
                    if(iter_inner.teamname == deletedteamname) {
                        iter_inner.teamname = DEFAULT_TEAMNAME;
                    }
                }
...
bool UserManager::modifyTeamName(int32_t userid,const std::string& newteamname,const std::string& oldteamname) {
...

    {
        ...
        for(auto& iter : m_allCachedUsers) {
            if(iter.userid == userid) {
                for(auto& iter_inner : iter.friends) {
                    if(iter_inner.teamname == oldteamname) {
                        iter_inner.teamname = newteamname;
                    }
                }
...
bool UserManager::updateMarknameInDb(int32_t userid,int32_t friendid,const std::string& newmarkname) {
    ...
    for(auto& iter : m_allCachedUsers) {
        if(iter.userid == userid) {
            for(auto& iter_inner : iter.friends) {
                if(iter_inner.friendid == friendid) {
                    iter_inner.markname = newmarkname;
                    return true;
                }
            }
        }
...
bool UserManager::getFriendInfoByUserId(int32_t userid,std::list<User>& friends) {
    ...
    //通过每个friendid找到对应的User集合
    //TODO: 效率低下
    for(const auto& iter : friendInfo) {
        User u;
        for(const auto& iter_inner : m_allCachedUsers) {
            if(iter_inner.userid == iter.friendid) {
                u = iter_inner;
                friends.push_back(u);
                break;
            }
...
bool UserManager::getFriendMarknameByUserId(int32_t userid,int32_t friendid,std::string& markname) {
    std::list<FriendInfo> friendInfo;
    std::unique_lock<std::mutex> guard(m_mutex);
    //先找到friends的ID列表
    for(const auto& iter : m_allCachedUsers) {
        if(iter.userid == userid) {
            friendInfo = iter.friends;
            for(const auto& iter_inner : friendInfo) {
                if(iter_inner.friendid == friendid) {
                    markname = iter_inner.markname;
                    return true;
                }
            }
        }
