...
void ChatSession::onRead(const std::shared_ptr<TcpConnection>& conn, Buffer* pBuffer,Timestamp receivTime) {
    while(true) {
        ...
        if(header.compressflag == PACKAGE_COMPRESSED) {
            //包头出错，立即关闭连接
            if(header.compressflag <= 0 || header.compresssize > MAX_PACKAGE_SIZE ||
        ...
        default:
        {
            if(m_isLogin) {
                switch(cmd) {
                    ...
                    default:
                    //  pBuffer->retrieveAll();
                        LOGE("upsupport cmd, cmd: %d, data=%s, connection name: %s",cmd,data.c_str(),conn->peerAddress().toIpPort().c_str());
                        //conn->forceClose();
                        return false;
                }//end inner-switch
            }else {
                //用户未登录，通知客户端不能进行操作，提示“未登录”
...
void ChatSession::onRegisterResponse(const std::string& data,const std::shared_ptr<TcpConnection>& conn) {
    ...
    if(!retData.empty()) {
        ...
        LOGI("Response to client: cmd=msg_type_register, data: %s, client: %s",retData,conn->peerAddress().toIpPort().c_str());
    }
}
...
void ChatSession::onLoginResponse(const std::string& data,const std::shared_ptr<TcpConnection>& conn) {
    ...
    if (cachedUser.userid == 0)
    {
        ...
        if(cachedUser.password != password) {
            ...
            m_userinfo.clienttype = clientType;
    ...
    LOGI("Response to client: cmd=msg_type_login, data=%s,userid=%d",os.str().c_str(),m_userinfo.userid);
    ...
    for(const auto& iter : friends) {
        //一个用户id，多个终端，则同一个userid可能对应多个session
        std::list<std::shared_ptr<ChatSession>> sessions;
        imserver.getSessionsByUserId(sessions,iter.userid);
        for(auto& iter_inner : sessions) {
            if(iter_inner) {
                iter_inner->sendUserStatusChangeMsg(m_userinfo.userid,1,m_userinfo.status);

                LOGI("sendUserStatusChangeMsg to user(userid: %d): user go online, online userid: %d, status: %d", iter_inner->getUserId(), m_userinfo.userid, m_userinfo.status);
            }
        }
    }
}
...
void ChatSession::onChangeUserStatusResponse(const std::string& data,const std::shared_ptr<TcpConnection>& conn) {
    ...
    for(const auto& iter : friends) {
        ...
        {
            if (iter_inner)
                iter_inner->sendUserStatusChangeMsg(m_userinfo.userid, 1, newstatus);
        }
    }
}
...
void ChatSession::onOperateFriendResponse(const std::string& data,const std::shared_ptr<TcpConnection>& conn) {
    ...
    if(targetUserid >= GROUPID_BOUNDARY) {
        if(type == 4) {
            //退群
    ...
    //应答加好友
        ...
        if(accept == 1) {
            if(!Singleton<UserManager>::Instance().makeFriendRelationShipInDB(targetUserid,m_userinfo.userid)) {
                LOGE("make relationship error: %s, userid: %d, client: %s",data.c_str(),conn->peerAddress().toIpPort().c_str());
                return;
            }
        ...
        if(sessions.empty()) {
            ...
            LOGI("userid: %d, is not online, cache notify msg, msg: %s",targetUserid,outbuf);
            return;
        }
...
void ChatSession::onAddGroupResponse(int32_t groupId,const std::shared_ptr<TcpConnection>& conn) {
    ...
    for(const auto& iter : friends) {
        ...
        for(auto& iter_inner : targetSessions) {
            if(iter_inner) {
                iter_inner->sendUserStatusChangeMsg(groupId,3);
            }
        }
    }
}
...
void ChatSession::onUpdateUserInfoResponse(const std::string& data,const std::shared_ptr<TcpConnection>& conn) {
    ...
    for(const auto& iter : friends) {
        ...
        for(auto& iter_inner : targetSessions) {
            if(iter_inner) {
                iter_inner->sendUserStatusChangeMsg(m_userinfo.userid,3);
            }
        }
    }
}
...
void ChatSession::sendUserStatusChangeMsg(int32_t userid,int type,int status) {
    string data;
    //用户上线
    if(type == 1) {
...
void ChatSession::deleteFriend(const std::shared_ptr<TcpConnection>& conn,int32_t friendid) {
    ...
    **/
   if(!Singleton<UserManager>::Instance().releseFriendRelationshipInDBAndMemory(friendid,m_userinfo.userid)) {
   ...
   //更新一下当前用户的分组信息
    ...
    for(const auto& iter : friends) {
        ...
        if(!targetSessions.empty()) {
            for(auto& iter_inner : targetSessions) {
                if(iter_inner) {
                    iter_inner->sendUserStatusChangeMsg(friendid,3);
                }
            }
        }
    }
...
void ChatSession::makeUpFriendListInfo(std::string& friendinfo,const std::shared_ptr<TcpConnection>& conn) {
    ...
    UserManager& userMgr = Singleton<UserManager>::Instance();
    ChatServer& imServer = Singleton<ChatServer>::Instance();
    userMgr.getTeamInfoByUserId(m_userinfo.userid,teaminfo);
    ...
    */
    ...
    userMgr.getUserInfoByUserId(m_userinfo.userid,currentUserInfo);
    ...
    for(auto& friendinfo : currentUserInfo.friends) {
        for(uint32_t i = 0;i < teamCount;++i) {
            ...
            if(!userMgr.getUserInfoByUserId(friendinfo.friendid,u)) {
                continue;
            }

            if(!userMgr.getFriendMarknameByUserId(m_userinfo.userid,friendinfo.friendid,markname)) {
                continue;
            }
...
void ChatSession::enableHeartbeatCheck() {
    std::shared_ptr<TcpConnection> conn = getConnectionPtr();
    if(conn) {
