...
FileSession::FileSession(const std::shared_ptr<TcpConnection>& conn,const char* basedir) 
:TcpSession(conn),
m_id(0),
m_seq(0),
m_FileBaseDir(basedir),
...
bool FileSession::process(const std::shared_ptr<TcpConnection>& conn,const char* inbuf,size_t length) {
    ...
    std::string filemd5;
    size_t md5length;
    if(!readStream.ReadString(&filemd5,0,md5length)) {
    ...
    //客户端文件上传
        ...
        case msg_type_download_req:
        {
            ...
            if(!readStream.ReadInt32(clientNetType)) {
                LOGE("read clientNetType error,close connection");
                return false;
            }

            return onDownloadFileResponse(filemd5,offset,conn);
        }
            break;
        default:
            LOGE("unsupported cmd, cmd: %d, client: %s",cmd,conn->peerAddress().toIpPort().c_str());
            conn->forceClose();
            return false;
    }
}
...
bool FileSession::onUploadFileResponse(const std::string& filemd5, int64_t offset, int64_t filesize, const std::string& filedata, const std::shared_ptr<TcpConnection>& conn) {
    ...
    if(offset == 0) {
        std::string filename = m_FileBaseDir;
        filename += filemd5;
